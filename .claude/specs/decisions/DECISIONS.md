...
| Jun 2026 | Broadened Rule 9 to allow `fetch_user_email` for the chat first-message notification | `public.users` has no email column, so the seller's address must be resolved from `auth.users`. Done as a post-response `BackgroundTask` (off the request/response path), never exposed to the client — same justification as the webhook exception. |
| Jun 2026 | `connect_args={"prepare_threshold": None}` on the async engine | DATABASE_URL targets the Supabase transaction pooler (:6543), which cannot retain psycopg3 server-side prepared statements across pooled connections. Without this, repeated queries fail with `DuplicatePreparedStatement`. |
| Jun 2026 | No DB foreign key from `public.users.id` to `auth.users(id)` | Alembic/app should not own a constraint into Supabase's managed `auth` schema, and test fixtures insert `public.users` rows directly (random UUIDs not in `auth.users`). Integrity is maintained by the `handle_new_user` trigger instead. Documented deviation from spec 06. |
| Jun 2026 | **Removed all payments / Razorpay** (migration 0006) | Platform processes no money. Buyers and sellers settle directly, offline. Eliminates payment SDK, Route payouts, seller KYC/onboarding, payment webhook, transaction statuses, amount/payout columns, the abandonment APScheduler job, and the 15-min payment window. **Supersedes every payment decision above and the payment specs.** |
| Jun 2026 | `transactions` reframed as verified-exchange records | A row exists only after a correct passkey at the meetup — it IS the completed transaction. Columns: `id, listing_id, buyer_id, seller_id, created_at`. `uq_transaction_per_listing` (partial, `listing_id IS NOT NULL`) enforces one sale per listing. Ratings still hang off `transaction_id`. |
| Jun 2026 | Passkey verification is the sole completion mechanism | `POST /transactions/verify-passkey` atomically marks the listing SOLD (winner-selection `UPDATE ... WHERE is_available=TRUE`), increments `books_sold`/`books_bought`, and lets the buyer rate. No payment confirmation involved. |
| Jun 2026 | `total_sales` → `books_sold`, added `books_bought` | First-class buyer + seller reputation counters, incremented atomically on verification (drift-checkable against the transactions table). |
| Jun 2026 | Repurposed `is_verified` as the verification badge | Blue badge auto-set TRUE once `books_sold >= 10` (in the verify path). Trigger no longer sets it from the OAuth email flag. Reuses the existing badge UI; supersedes the Month-1 "`is_verified` = OAuth email verified" decision. |
| Jun 2026 | Ratings: buyer-only, once, optional `review` text | Only the buyer of a verified transaction can rate, enforced by `UNIQUE(transaction_id, rated_by)` + an ownership check. `seller_rating` recomputed as `AVG(rating)` on each insert. All ratings are inherently from verified transactions. |
| Jun 2026 | Signup welcome email via APScheduler sweep on `users.welcome_email_sent` | There are no auth endpoints on FastAPI, so the only signup signal is the `public.users` row the `handle_new_user` trigger inserts. A 5-min sweep (migration 0008 adds the flag) emails new rows once and flips the flag — permanent, so sign-out/sign-in never resends. Existing users backfilled to TRUE so the first sweep doesn't email the current base. `fetch_user_email` (service role) is used inside this background job, which is already permitted by the primary clause of Security Rule 7 ("background jobs only") — not a new request-path exception, so Rule 7's exception list (a/b) is unchanged. |
| Jun 2026 | College communities = canonical `colleges` table + FK (deviates from the city/state denormalized-TEXT pattern) | Spec 15. `/colleges/[slug]` needs stable slugs and the campus list must grow via the Supabase dashboard with no code deploy — a hardcoded igod-style constant can't. College is **discovery-only** (no members/feed/posts/group chat/notifications) and is **never** tied to `is_verified` (self-asserted, like city). Un-listed campuses are captured as free-text `college_other` (holding pen; `college_id IS NULL`) and promoted to canonical rows via manual dashboard SQL + backfill; `is_active=FALSE` retires one without deleting references. App enforces at-most-one of (college_id, college_other). |